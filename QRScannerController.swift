...
class QRScannerController: UIViewController {
    ...
    private let supportedCodeTypes = [AVMetadataObject.ObjectType.ean13]
    
    override func viewDidLoad() {
        super.viewDidLoad()
        // Get the back-facing camera for capturing videos
        /* let deviceDiscoverySession = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInDualCamera], mediaType: AVMediaType.video, position: .back) */
        //PROBLEM: no way of using more advanced cameras, Dual Angle
    ...
    func launchApp(decodedURL: String) {
        ...
        let alertPrompt = UIAlertController(title: "Barcode Detected!", message: "Found ISBN: \(decodedURL)", preferredStyle: .actionSheet)
        //var url = "https://www.googleapis.com/books/v1/volumes?q=isbn:" + decodedURL
        let confirmAction = UIAlertAction(title: "Confirm", style: UIAlertActionStyle.default, handler: { (action) -> Void in
              if let url = URL(string: decodedURL) {
                //if UIApplication.shared.canOpenURL(url) {
                    //UIApplication.shared.open(url, options: [:], completionHandler: nil)
                //}
                            self.getBookInfo(isbn: decodedURL)
            }
           //9780545010221
        })
    ...
    func getBookInfo(isbn: String) {
        ...
        URLSession.shared.dataTask(with: url, completionHandler: {data, response, error -> Void in
            ...
            if let arrayOfPublishedDate = (jsonResult as AnyObject).value(forKeyPath: "items.volumeInfo.publishedDate") as? [[String]] {
                print(arrayOfPublishedDate)
            }
        }).resume()
    }
}
